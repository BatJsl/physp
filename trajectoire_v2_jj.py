...
def système(t, variables):
    ...
    Cd = 2.44e-3 - 1.09e-3 * twt + 3.03e-4 * twt ** 2 + 3.59e-7 * twt ** 3 - 2.50e-8 * twt ** 4  # coefficient de trainée
    Cl = 6.25e-3 * twt + 2.41e-4 * twt ** 2 - 3.44e-6 * twt ** 3  # coefficient de portance
    Cm = 1.51e-2 * twt - 1.69e-4 * twt ** 2
    Cy = twt * (-1.50e-3 + 6.49e-4 * w - 8.35e-5 * w ** 2) + (-4.11e-5 - 3.82e-5 * w + 2.64e-6 * w ** 2) * twt ** 2 + (
                4.94e-7 + 2.74e-7 * w - 1.77e-8 * w ** 2) * twt ** 3
    ...
    Mij = np.array([[cos(THET) * cos(PSI), sin(THET) * cos(PSI) * sin(PHI) - cos(PHI) * sin(PSI),
                     cos(PHI) * cos(PSI) * sin(THET) + sin(PHI) * sin(PSI)],
                    [cos(THET) * sin(PSI), sin(THET) * sin(PSI) * sin(PHI) + cos(PHI) * cos(PSI),
                     cos(PHI) * sin(PSI) * sin(THET) - sin(PHI) * cos(PSI)],
                    [-sin(THET), sin(PHI) * cos(THET), cos(PHI) * cos(THET)]], dtype=float)
